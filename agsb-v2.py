...
# 添加命令行参数解析
def parse_args():
    parser = argparse.ArgumentParser(description="ArgoSB Python3 一键脚本 (支持自定义域名)")
    ...
    parser.add_argument("--domain", "-d", help="设置自定义域名 (例如: xxx.trycloudflare.com)")
    parser.add_argument("--uuid", "-u", help="设置自定义UUID")
    parser.add_argument("--port", "-p", type=int, help="设置自定义端口")
...
def print_info():
    ...
    print("\033[36m│ \033[32m版本: 25.6.1 (自定义域名版)                                \033[36m│\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")

# 打印使用帮助信息
def print_usage():
    print("\033[33m使用方法:\033[0m")
    print("  \033[36mpython3 agsb_custom_domain.py\033[0m              - 安装并启动服务")
    print("  \033[36mpython3 agsb_custom_domain.py install\033[0m      - 安装服务")
    print("  \033[36mpython3 agsb_custom_domain.py --domain example.com\033[0m - 使用自定义域名安装")
    print("  \033[36mpython3 agsb_custom_domain.py status\033[0m       - 查看服务状态和节点信息")
    print("  \033[36mpython3 agsb_custom_domain.py cat\033[0m          - 查看单行节点列表")
    print("  \033[36mpython3 agsb_custom_domain.py update\033[0m       - 更新脚本")
    print("  \033[36mpython3 agsb_custom_domain.py del\033[0m          - 卸载服务")
    print()
    print("\033[33m支持的环境变量:\033[0m")
    print("  \033[36mexport vmpt=12345\033[0m                       - 设置自定义端口")
...
def generate_links(domain, port_vm_ws, uuid_str):
    ...
    # 保存域名到文件
    domain_file = INSTALL_DIR / "custom_domain.txt"
    with open(str(domain_file), 'w') as f:
        f.write(domain)
    ...
    with open(str(LIST_FILE), 'w') as f:
        ...
        f.write("\033[36m│ \033[32m查看节点信息: \033[0mpython3 agsb_custom_domain.py status\n")
        f.write("\033[36m│ \033[32m查看所有节点(一行一个): \033[0mpython3 agsb_custom_domain.py cat\n")
        f.write("\033[36m│ \033[32m升级脚本: \033[0mpython3 agsb_custom_domain.py update\n")
        f.write("\033[36m│ \033[32m卸载脚本: \033[0mpython3 agsb_custom_domain.py del\n")
        f.write("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m\n")
    ...
    with open(str(LIST_FILE) + ".txt", 'w') as f:
        ...
        f.write("使用方法:\n")
        f.write("查看节点信息: python3 agsb_custom_domain.py status\n")
        f.write("查看所有节点(一行一个): python3 agsb_custom_domain.py cat\n")
        f.write("升级脚本: python3 agsb_custom_domain.py update\n")
        f.write("卸载脚本: python3 agsb_custom_domain.py del\n")
        f.write("---------------------------------------------------------\n")
    ...
    with open(str(readme_file), 'w') as f:
        ...
        f.write("```bash\npython3 agsb_custom_domain.py cat\n```\n\n")
    ...
    print("\033[36m│ \033[32m使用 \033[33mpython3 agsb_custom_domain.py status\033[32m 查看节点信息\033[0m")
    print("\033[36m│ \033[32m使用 \033[33mpython3 agsb_custom_domain.py cat\033[32m 查看单行节点\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
...
def install(args):
    ...
    # 获取UUID - 优先使用命令行参数，其次环境变量，最后随机生成
    if args.uuid:
        uuid_str = args.uuid
        print(f"使用命令行指定的UUID: {uuid_str}")
    elif os.environ.get("uuid"):
        uuid_str = os.environ.get("uuid")
        print(f"使用环境变量指定的UUID: {uuid_str}")
    else:
        uuid_str = str(uuid.uuid4())
        print(f"使用随机生成的UUID: {uuid_str}")
    
    # 获取端口 - 优先使用命令行参数，其次环境变量，最后随机生成
    if args.port:
        port_vm_ws = args.port
        print(f"使用命令行指定的端口: {port_vm_ws}")
    elif os.environ.get("vmpt"):
        port_vm_ws = int(os.environ.get("vmpt"))
        print(f"使用环境变量指定的端口: {port_vm_ws}")
    else:
        port_vm_ws = random.randint(10000, 65535)  # 随机生成端口
        print(f"使用随机生成的端口: {port_vm_ws}")
    ...
    # 获取域名 - 优先使用命令行参数，其次环境变量，最后尝试自动获取
    custom_domain = None
    if args.domain:
        custom_domain = args.domain
        print(f"使用命令行指定的域名: {custom_domain}")
    elif os.environ.get("agn"):
        custom_domain = os.environ.get("agn")
        print(f"使用环境变量指定的域名: {custom_domain}")
    ...
    if custom_domain:
        ...
        generate_links(custom_domain, port_vm_ws, uuid_str)
    else:
        # 尝试自动获取域名
        ...
        if domain:
            ...
            print("无法获取tunnel域名，请手动指定域名:")
            print("  方法1: python3 agsb_custom_domain.py --domain your-domain.com")
            print("  方法2: export agn=your-domain.com && python3 agsb_custom_domain.py")
            sys.exit(1)
...
def upgrade():
    try:
        script_content = http_get("https://raw.githubusercontent.com/yonggekkk/argosb/main/agsb_custom_domain.py")
        if script_content:
...
def check_status():
    try:
        ...
        if sing_box_running and cloudflared_running and os.path.exists(str(LIST_FILE)):
            ...
            # 检查自定义域名文件
            domain_file = INSTALL_DIR / "custom_domain.txt"
            if os.path.exists(str(domain_file)):
                with open(str(domain_file), 'r') as f:
                    custom_domain = f.read().strip()
                print(f"\033[36m│ \033[32m自定义域名: \033[0m{custom_domain}")
            else:
                # 读取临时域名
            ...
            print("\033[36m│ \033[32m运行 \033[33mpython3 agsb_custom_domain.py\033[32m 开始安装\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            return False
        else:
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            ...
            print("\033[36m│ \033[32m建议卸载后重新安装: \033[33mpython3 agsb_custom_domain.py del\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            return False
    except Exception as e:
        print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
...
def main():
    ...
    if args.action == "install":
        install(args)
    elif args.action in ["uninstall", "del", "delete", "remove"]:
        uninstall()
        ...
        if check_status():
            ...
            print("\033[36m│ \033[32m卸载命令: \033[33mpython3 agsb_custom_domain.py del\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
        else:
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
